/* =============================================
   ...
   shared multipliers in lib/recognition-pricing.js (standard 1.0 / premium 1.5 /
...
(function (root, factory) {
  ...
  var MULTIPLIERS = (Pricing && Pricing.KIT_MULTIPLIERS) || { standard: 1.0, premium: 1.5, enterprise: 1.95 };

  // Standard (×1.0) base prices. MUST match shop.html / product.html.
  var BASE_PRODUCTS = {
    'das-001': { name: 'Driver Appreciation Kit',     basePrice: 249, minQty: 10 },   // 49.99 -> 249 per Jayden 2026-08-27
    'das-003': { name: 'Premium Onboarding Pack',     basePrice: 59.99, minQty: 10 },
    'das-004': { name: 'Holiday Gift Set',            basePrice: 54.99, minQty: 10, unavailable: true }, // seasonal — available Sept 21 ($1 live-payment test reverted 2026-08-20)
  ...
  // Tier price in INTEGER CENTS - 49.99*1.5 floats to 74.98499..., which
  // round2 truncates to 74.98, a price published nowhere (the site advertises
  // $74.99 in seven places). Math.round(4999*1.5) = 7499 -> 74.99.
  function tierPrice(basePrice, mult) { return Math.round(Math.round(Number(basePrice) * 100) * Number(mult)) / 100; }
  ...
  // The tier named by the id suffix ('das-001-premium' → 'premium'); null when untiered.
  function tierOf(id) {
    var m = String(id == null ? '' : id).match(/-(standard|premium|enterprise)$/);
    return m ? m[1] : null;
  }
  ...
  function allowedPrices(basePrice) {
    ...
    for (var k in MULTIPLIERS) {
      if (Object.prototype.hasOwnProperty.call(MULTIPLIERS, k)) out.push(tierPrice(basePrice, MULTIPLIERS[k]));
    }
    return out;
  }
  ...
  function resolve(item) {
    ...
    var tier = tierOf(item.id);
    if (tier) {
      // Tier-suffixed id: exactly ONE legitimate price. Previously any of the
      // three tier prices verified for any tier id, so 'das-001-premium' could
      // check out at the standard price while the (client-controlled) line-item
      // name still said Premium.
      var want = tierPrice(known.basePrice, MULTIPLIERS[tier] || 1);
      if (Math.abs(want - clientPrice) < 0.011) {
        var tierLabel = tier.charAt(0).toUpperCase() + tier.slice(1);
        return { status: 'verified', unitPrice: want, minQty: known.minQty, name: known.name + ' — ' + tierLabel + ' Tier' };
      }
      return { status: 'rejected', reason: 'price_mismatch', allowed: [want], minQty: known.minQty };
    }
