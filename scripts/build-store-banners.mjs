...
writeFileSync(join(OUT, 'banner-manifest.json'), JSON.stringify({ banner: BANNER, banners: manifest }, null, 1))
...
lines.push('')
lines.push('## 1 · Programme banners — the biggest gap')
lines.push('')
lines.push('The store home runs a full-bleed banner above each programme\'s product row, matching')
lines.push('shop.griffain.io. Every banner is **2400 x 820px (2.93:1 landscape)**. Nothing in the')
lines.push('library is anywhere near that shape, so these are rendering as branded placeholders')
lines.push('that name the programme and say a photograph is coming.')
lines.push('')
